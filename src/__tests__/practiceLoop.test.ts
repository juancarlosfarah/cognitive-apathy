...
describe('practiceLoop', () => {
  test('should repeat the timeline if keys were released early or number of taps is less than MINIMUM_CALIBRATION_MEDIAN', async () => {
    ...
    jsPsych.data = mockData as any;

    (checkFlag as jest.Mock).mockImplementation((task, flag) => {
  ...
  test('should not repeat the timeline if conditions are not met', async () => {
    const jsPsych = initJsPsych();
