...
function handleCalibrationFailLogic(
  ...
  const numTrialsMap: Record<string, number> = {
  ...
  const successesKey = `${calibrationPart}Successes` as keyof State;
  state[successesKey] = (state[successesKey] as number) + 1;
...
function handleCalibrationLoopLogic(  
  calibrationPart: string,
  state: State) 
  {
